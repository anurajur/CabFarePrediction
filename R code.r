...
# Lode directory
setwd("E:/Edwisor assignments/Edwisor Project")
getwd()
...
str(df)
df$Trip_distance_KM[df$Trip_distance_KM <= 0] = NA

df$Trip_distance_KM[df$Trip_distance_KM > 200] = NA

sum(is.na(df))

df$Trip_distance_KM[is.na(df$Trip_distance_KM)] =
  median(df$Trip_distance_KM, na.rm = T)
...
## We will first get a basic idea of how different models perform on our preprocesed data and then
